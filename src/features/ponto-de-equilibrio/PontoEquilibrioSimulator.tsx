...
import {
  DEFAULT_AVERAGE_VARIABLE_COST_PERCENT,
  DEFAULT_DESIRED_MONTHLY_PROFIT,
  DEFAULT_WORKING_DAYS_PER_MONTH,
  generatePontoEquilibrioNarrative,
  runPontoEquilibrioSimulator,
} from '@/features/ponto-de-equilibrio'
import type {
...
const INPUT_CLASS =
  'w-full rounded-xl border border-white/[0.08] bg-slate-900/60 px-4 py-2.5 text-sm text-slate-50 placeholder:text-slate-600 transition-all duration-200 focus:border-amber-500/50 focus:outline-none focus:ring-2 focus:ring-amber-500/20'

const LABEL_CLASS = 'mb-1.5 block text-xs font-medium text-slate-400'

// ── Mapeamentos visuais ────────────────────────────────────────────────

const SEVERITY_TO_TONE: Record<PontoEquilibrioDiagnosticSeverity, ToolTone> = {
  success: 'positive',
  info: 'neutral',
  warning: 'attention',
  danger: 'critical',
...
function PlaybookCard({ playbook }: { playbook: PontoEquilibrioPlaybook }) {
...
interface FormState {
  ...
  averageVariableCostPercent: string
...
export function PontoEquilibrioSimulator() {
  const [form, setForm] = useState<FormState>({
    monthlyFixedCosts: '',
    averageVariableCostPercent: String(DEFAULT_AVERAGE_VARIABLE_COST_PERCENT),
    averageTicket: '',
  ...
  const [result, setResult] = useState<PontoEquilibrioOrchestratorResult | null>(null)
  const [narrative, setNarrative] = useState<PontoEquilibrioNarrative | null>(null)
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const variablePct = parseFloat(form.averageVariableCostPercent)
    const ticket = parseFloat(form.averageTicket)
    const currentRev = form.currentMonthlyRevenue.trim()
      ? parseFloat(form.currentMonthlyRevenue)
      : undefined
    const profit = form.desiredMonthlyProfit.trim()
      ? parseFloat(form.desiredMonthlyProfit)
      : 0
    const workDays = form.workingDaysPerMonth.trim()
      ? parseFloat(form.workingDaysPerMonth)
      : DEFAULT_WORKING_DAYS_PER_MONTH
    ...
    if (isNaN(variablePct)) {
      setError('Informe um percentual válido para o custo variável médio.')
      return
    }
    if (isNaN(ticket) || ticket <= 0) {
      setError('O ticket médio deve ser maior que zero.')
      return
    }
    if (currentRev !== undefined && currentRev <= 0) {
      setError('O faturamento atual deve ser maior que zero quando informado.')
      return
    }
    if (isNaN(profit) || profit < 0) {
      setError('A meta de lucro deve ser maior ou igual a zero.')
      return
    }
    if (isNaN(workDays) || workDays < 1 || workDays > 31) {
      setError('Os dias úteis por mês devem ser entre 1 e 31.')
      return
    }

    try {
      const simResult = runPontoEquilibrioSimulator({
        monthlyFixedCosts: fixedCosts,
        averageVariableCostPercent: variablePct,
        averageTicket: ticket,
        currentMonthlyRevenue: currentRev,
        desiredMonthlyProfit: profit,
        workingDaysPerMonth: workDays,
      })
      setResult(simResult)
      setNarrative(generatePontoEquilibrioNarrative(simResult))
    } catch (err) {
      setError(
    ...
    <div className="mt-6">
      ...
      <div className="rounded-2xl border border-white/[0.08] bg-slate-900/50 p-6 backdrop-blur-sm sm:p-8">
        ...
        <p className="mb-6 text-sm text-slate-400">
          Informe os custos e parâmetros financeiros para calcular o faturamento mínimo e a meta com lucro.
        </p>
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            {/* Ticket médio */}
            <div>
              <label htmlFor="averageTicket" className={LABEL_CLASS}>
            ...
            <div className="sm:col-span-2">
              <label htmlFor="averageVariableCostPercent" className={LABEL_CLASS}>
                Custo variável médio (%)
              </label>
              <input
                id="averageVariableCostPercent"
                type="number"
                inputMode="decimal"
                min="0"
                max="94"
                step="0.1"
                value={form.averageVariableCostPercent}
                onChange={field('averageVariableCostPercent')}
                className={INPUT_CLASS}
              />
            </div>
            ...
            {/* Faturamento atual */}
            <div>
              <label htmlFor="currentMonthlyRevenue" className={LABEL_CLASS}>
              ...
              <input
                ...
                onChange={field('currentMonthlyRevenue')}
                className={INPUT_CLASS}
              />
            </div>
          ...
          <button
            type="submit"
            className="mt-6 inline-flex h-11 w-full items-center justify-center rounded-xl bg-amber-600 px-6 text-sm font-semibold text-white shadow-[0_0_0_0_rgba(245,158,11,0)] transition-all duration-300 hover:bg-amber-500 hover:shadow-[0_0_20px_rgba(245,158,11,0.3)] active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-2 focus-visible:ring-offset-[#020617] sm:w-auto"
          >
        ...
        <div ref={resultRef} aria-live="polite" className="mt-10 scroll-mt-24">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
            <ToolMetricCard
              ...
              value={formatCurrency(result.calculation.breakEvenRevenue)}
              accent
            />
            <ToolMetricCard
              label="Receita alvo com lucro"
              value={formatCurrency(result.calculation.targetRevenue)}
            />
            <ToolMetricCard
              label="Procedimentos para empatar"
              value={formatCount(result.calculation.breakEvenProcedures)}
            />
            <ToolMetricCard
              label="Procedimentos para meta"
              value={formatCount(result.calculation.targetProcedures)}
            />
          </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
            <ToolMetricCard
              label="Faturamento diário mínimo"
              value={formatCurrency(result.calculation.dailyBreakEvenRevenue)}
            />
            <ToolMetricCard
              label="Faturamento diário alvo"
              value={formatCurrency(result.calculation.dailyTargetRevenue)}
            />
            <ToolMetricCard
              label="Margem de contribuição"
              value={formatPercent(result.calculation.contributionMarginPercent)}
              warning={result.calculation.contributionMarginPercent < 40}
            />
            <ToolMetricCard
              label="Custo variável médio"
              value={formatPercent(result.calculation.averageVariableCostPercent)}
            />
            <ToolMetricCard
              label="Ticket médio"
              value={formatCurrency(result.calculation.averageTicket)}
            />
            {result.calculation.currentMonthlyRevenue !== undefined && (
              <>
                <ToolMetricCard
                  label="Gap atual (atual − breakeven)"
                  value={formatCurrency(result.calculation.revenueGapAmount ?? 0)}
                  warning={(result.calculation.revenueGapAmount ?? 0) < 0}
                />
                {result.calculation.revenueGapPercent !== undefined && (
                  <ToolMetricCard
                    label="Gap percentual"
                    value={formatPercent(result.calculation.revenueGapPercent)}
                    warning={result.calculation.revenueGapPercent < 0}
                  />
                )}
                {result.calculation.safetyMarginPercent !== undefined && (
                  <ToolMetricCard
                    label="Margem de segurança"
                    value={formatPercent(result.calculation.safetyMarginPercent)}
                    warning={result.calculation.safetyMarginPercent < 5}
                  />
                )}
              </>
            )}
